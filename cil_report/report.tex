...
In the SVD with dimension reduction method, we keep the $k$ most significant singular values for the reproduction of the rating matrix. $$reproduction of the matrix$$ For each movie $j$, the missing values in its column are filled with the mean of its observed ratings. $k$ is set to 12 in our system.
